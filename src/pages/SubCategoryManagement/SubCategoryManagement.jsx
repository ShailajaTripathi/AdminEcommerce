...
import { subcategoryaddedit } from '../../config/routingConsts'
...
const SubCategoryManagement = () => {
                    ...
                    switch(row.row.original.status){
                        case 7:
                            return(
                                ...
                                <div className={`table-action ${row?.row?.original?.categoryId === openAction ? "active" : ""}`}>
                                    <div className={`action-btn ${row?.row?.original?.categoryId === openAction ? "active" : ""}`} onClick={() => OpenAction(row?.row?.original?.categoryId)}><IconsDots/></div>
    ...
    async function deletecategoryId(categoryId) {
        return await requestApi.post(`category/delete`, {
            categoryId
        })
    }
    ...
    function handlePermission(categoryId) {
        mutationStatusUpdate.mutate(categoryId)
        setShowMaterialStatusPopup(false)
    }

    async function updateStatusbyId(categoryId) {
        if(buttonText=="Approve"){
            return await requestApi.post(`/category/approve-decline`, {
                categoryId,
                status:1
            })
        }else{
    ...
    const onStatusChange = useCallback((categoryId) => {
        mutationStatusActiveInactive.mutate(categoryId)
    }, [])
    async function updateStatusbycategoryId(categoryId) {
        return await requestApi.post(`/category/active-inactive-status`, {
            categoryId
        })
    }
                ...
                <div class="head">
                    ...
                    <div className="search-btn-group">
                        ...
                        >
                            ...
                            <Dropdown
                            flip="no"
                            onSelect={(e)=>setMainCategory(JSON.parse(e))}
                            name="MainCategory"
                            drop="down-centered"
                        ...
                        >
                            ...
                            <Dropdown.Menu>
                            <Dropdown.Item eventKey={JSON.stringify({name:""})}>
                                           All
                                        </Dropdown.Item>
                        ...
                        <button
                            ...
                            onClick={() => navigate(subcategoryaddedit)}
                        >
