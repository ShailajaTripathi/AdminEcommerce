...
import { designerview } from '../../config/routingConsts'
...
const DesignerManagement = () => {
    ...
    async function deleteMaterialId(sellerId) {
    ...
    const columns = [
        ...
        columnHelper.accessor((row) => row?.sellerId, {
            id: 'sellerId',
            header: () => 'Id',
            cell: (row) => {
                return (
                    <span>
                        {row?.row?.original?.sellerId
                            ? row?.row?.original?.sellerId
                            : '-'}
                    </span>
                )
        ...
        columnHelper.accessor((row) => row?.storeName, {
            id: 'storeName',
            header: () => 'Store Name',
            cell: (row) => {
                return (
                    <span>
                        {row?.row?.original?.storeName
                            ? row?.row?.original?.storeName
                            : '-'}
                    </span>
                )
        ...
        columnHelper.accessor((row) => row?.phoneNo, {
            ...
            cell: (row) => {
                return (
                    <span>
                        {row?.row?.original?.phoneNo
                            ? '+' +
                              row?.row?.original?.countryCode +
                              ' ' +
                              row?.row?.original?.phoneNo
                            : '-'}
                    </span>
        ...
        columnHelper.accessor((row) => row?.totalProduct, {
            id: 'totalProduct',
            header: () => 'Total Products',
            cell: (row) => {
                return (
                    <span>
                        {row?.row?.original?.totalProduct
                            ? <b>{row?.row?.original?.totalProduct}</b>
                            : '-'}
                    </span>
                )
            }
                            ...
                            {row?.row?.original?.status == 7 ? (
                                <>
                                    {/* <div
                                        ...
                                        onClick={() =>
                                            handleApproveDecline(row?.row?.original?.sellerId)
                                        }
                                    >
                                        <DeclineIcon />
                                    </div>
                                    <div
                                        className="action-icon"
                                        onClick={() => {handleApproveDecline(row?.row?.original?.sellerId)
                                        }}
                                    >
                                        <ApproveIcon />
                                    </div> */}

                                <div className={`table-action ${row?.row?.original?.sellerId === openAction ? "active" : ""}`}>
                                    <div className={`action-btn ${row?.row?.original?.sellerId === openAction ? "active" : ""}`} onClick={() => OpenAction(row?.row?.original?.sellerId)}><IconsDots/></div>
                                    ...
                                    >
                                        ...
                                        <div className="action-panel two-action">
                                            ...
                                            <div
                                                ...
                                                onClick={() => {
                                                     navigate(`${designerview}/${row?.row?.original?.sellerId}`)
                                                }}
                                            >
                                                <IconView />
                                            </div>
    ...
    async function updateStatusbyId(materialId) {
        return await requestApi.post(`/material/approve`, {
            materialId
        })
    }
                ...
                <div className="admin-card">
                    <div class="head">
                        ...
                        <div className="search-btn-group">
                            ...
                            {/* <button onClick={()=>navigate(materialaddedit)}  className='solid-red-btn'>Add New Vendor</button> */}
                        </div>
                    </div>
                </div>
                        ...
                        <div className="pagination-wrap">
                            <div className="showing-pages">
                                <span>
                                    ...
                                    of {data.meta.totalCount} Designers
                                </span>
                            </div>
